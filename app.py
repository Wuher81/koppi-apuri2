...
def aja_haku_varmistettu(user, pw, alku_pvm, loppu_pvm):
    ...
    try:
        ...
        time.sleep(2) # Viive palvelimen päivitystä varten
        
        # 3. HYPYN YRITYS: Ohjataan suoraan Ässät-sovellukseen (Portal 2)
        # Tämä vastaa "TO BROWSER VERSION" -painikkeen klikkaamista
        st.write("Siirrytään selainversioon...")
        # Haetaan sivu, joka pakottaa portaalivalinnan
        session.get("https://login.jopox.fi/home/select?to=145&portal=2", allow_redirects=True)
        time.sleep(2)
        
        # 4. LOPULLINEN VARMISTUS
        # Kokeillaan päästäänkö kotisivulle ilman, että meidät ohjataan takaisin kirjautumiseen
        final_check = session.get("https://assat-app.jopox.fi/home", allow_redirects=True)
        
        if "Kirjaudu" in final_check.text or "login" in final_check.url:
            st.error("❌ Kirjautuminen epäonnistui. Varmista tunnus ja salasana.")
            return None
                ...
                for event in events:
                    if pvm_str in event:
                        ...
                        if uid_match:
                            ...
                            # Lasketaan pelaajat (chip-luokka, jonka sisällä on player-luokka)
                            # Tämä perustuu lähettämääsi HTML-lähdekoodiin
        ...
        with st.status("Haetaan tietoja...", expanded=True) as status:
            ...
            if data is not None:
                if data:
                    ...
                    status.update(label="Ei tapahtumia.", state="error")